...
A few google searches were conducted to add states, cities, and facility names that were missing. Chatgpt was used to get one code in order to clean up the facility name that had either open or close bracket withoot the other. The prompt used was: For a column containing names, remove brackets if there is a start ["("] but no end [")"] and end [")"] but no start ["("]. If both open and close brackets are present, ignore them and keep them as they are
